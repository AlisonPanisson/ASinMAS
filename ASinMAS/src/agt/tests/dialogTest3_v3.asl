...
has_argument_against(Content,Justification):- incompatible(conclusion(Content),Comp) & argument(Comp,Justification) & not(.empty(Justification)).
...
//+!has_argument(X): has_argument(X,Arg) <-  .print(Arg).
//+!has_argument(X) <-  .print("Não").
...
+!responseAssertContent(Sender,Content[dialogue(Dialogue),source(Source)]): has_argument(conclusion(Content),Arg) & .my_name(MyName) & .print("Received Message:  ASSERT(",Sender, ",", MyName, "," , Content,")")
	<-	//.print("## I have an argument for it: ", Arg);
		performatives.send(Sender,accept,Content[dialogue(Dialogue)]).
	
+!responseAssertContent(Sender,Content[dialogue(Dialogue),source(Source)]):  incompatible(conclusion(Comp),conclusion(Content)) & argument(Comp,Justification) & .my_name(MyName) & .print("Received Message:  ASSERT(",Sender, ",", MyName, "," , Content,")")
	<-	//.print("## I have an argument against it: ", Arg);
		performatives.send(Sender,question,Content[dialogue(Dialogue)]).
...
+!responseJustify(Sender,[defeasible_rule(conclusion(C),S)[as(AS)]|Premises],Dialogue): .my_name(MyName) & .print("Received Message:  JUSTIFY(",Sender, ",", MyName, ","  ,[defeasible_rule(conclusion(C),S)[as(AS)],Premises],")")  
	<-  //.print("I have no argument for or against it: ", Dialogue);
		performatives.send(Sender,accept,C[dialogue(Dialogue)]);
		!check_subject(Sender,Dialogue).
...
+!respondQuestion(Sender,Content,D): argument(conclusion(Content),Justification) & .my_name(MyName) & .print("Received Message:  QUESTION(",Sender, ",", MyName, ","  ,Content,")")
...
+!talk_about([First|Rest],Dialogue): has_argument_against(First,Arg) & incompatible(conclusion(First),conclusion(Comp)) 
	<- 	-+list_cqs2discuss(Rest)
		!send(Sender,assert,Comp[dialogue(Dialogue)]).

+!talk_about([First],Dialogue): has_argument_against(First,Arg) & incompatible(conclusion(First),conclusion(Comp)) 
	<- 	-+list_cqs2discuss(Rest)
		!send(Sender,assert,Comp[dialogue(Dialogue)]).
